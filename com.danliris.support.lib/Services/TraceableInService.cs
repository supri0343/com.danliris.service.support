    ...
    public class TraceableInService
    {
        ...
        public IQueryable<TraceableINViewModel> getQueryTracable(string filter, string tipe, DateTime? Datefrom, DateTime? DateTo)
        {
            ...
            try
            {
                ...
                {
                    TraceableINViewModel dup = Array.Find(b, o => o.BCType == a.BCType && o.BCNo == a.BCNo && o.BonNo == o.BonNo);
                    if (dup != null)
                    {
                        if (dup.count == 0)
                        {
        ...
        {
            var Query = getQueryTracable(filter, tipe, Datefrom, DateTo);
            ...
            else
            {
                ...
                foreach (var item in Query)
                {
                    ...
                    result.Rows.Add(item.count, item.BCType, item.BCNo, bcdate, item.BonNo, item.PO, item.ItemCode, item.ItemName, item.ReceiptQty, item.SatuanReceipt, item.BUK, item.QtyBUK, item.SatuanBUK, item.ProduksiQty, item.BJQty, item.Invoice, item.PEB, pebdate, item.EksporQty, item.SampleQty);
