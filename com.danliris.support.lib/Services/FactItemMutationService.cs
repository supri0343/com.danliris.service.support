            ...
            try
            {
                    ...
                    new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        "declare @StartDate datetime = '"+ DateFrom + "' "+
                        "declare @EndDate datetime = '"+ DateTo + "' " +
						"declare @balanceDate1 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=1 and classificationid='BP'  order by d.CreateDate desc)  " +
						"declare @balanceDate2 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=2 and classificationid='BP'    order by d.CreateDate desc)  " +
						"declare @balanceDate3 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=3 and classificationid='BP'  order by d.CreateDate desc) " +
						"declare @balanceDate4 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId   join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=4 and classificationid='BP' order by d.CreateDate desc) " +
						"declare @balanceDate5 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=5 and classificationid='BP'  order by d.CreateDate desc)  " +
						 "select data.unitCode,data.ItemCode,ItemName, UnitQtyName, Convert(float, SUM(BeginQty)) as BeginQty, Convert(float, SUM(ReceiptQty)) ReceiptQty, Convert(float, SUM(ExpenditureQty)) ExpenditureQty, Convert(float, SUM(AdjustmentQty)) AdjustmentQty, Convert(float, SUM(OpnameQty)) as OpnameQty into #balance from( " +
            ...
            try
            {
                    ...
                    new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        "declare @StartDate datetime = '" + DateFrom + "' " +
                        "declare @EndDate datetime = '" + DateTo + "' " +
						"declare @balanceDate1 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=1 and classificationid='BB'  order by d.CreateDate desc)  " +
						"declare @balanceDate2 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=2 and classificationid='BB'    order by d.CreateDate desc)  " +
						"declare @balanceDate3 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=3 and classificationid='BB'  order by d.CreateDate desc) " +
						"declare @balanceDate4 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId   join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=4 and classificationid='BB' order by d.CreateDate desc) " +
						"declare @balanceDate5 datetime=(select top(1) d.CreateDate from BalanceStock_Temp d join Stock_Temp s on d.StockId=s.StockId join ITEM_CATEGORY i on i.ItemCode=s.ItemCode join dimcategory c on c.categoryId=i.CategoryId where s.UnitCode=5 and classificationid='BB'  order by d.CreateDate desc)  " +
                        "select data.unitCode,data.ItemCode,ItemName, UnitQtyName, Convert(float, SUM(BeginQty)) as BeginQty, Convert(float, SUM(ReceiptQty)) ReceiptQty, Convert(float, SUM(ExpenditureQty)) ExpenditureQty, Convert(float, SUM(AdjustmentQty)) AdjustmentQty, Convert(float, SUM(OpnameQty)) as OpnameQty into #balance from( " +
