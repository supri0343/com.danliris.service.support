			...
			try
			{
					...
					new SqlConnection(connectionString))
				{
					...
					" select * into #returexpend from ( select ComodityId, sum(case when ProcessDate < @DateFrom then Qty else 0 end) as SaldoQtyFin, 0 as QtyFin, 0 as FinishingTransfer, 0 as AdjFin, sum(case when ProcessDate >= @DateFrom then Qty else 0 end) as Retur, 0 as QtyExport, 0 as QtySample, 0 as QtyOther, 0 as sewingretur, 0 as QtyFinSub, 0 as cuttingSubkon, 0 as finoutSubkon from factreturExpend where ProcessDate <= @DateTo group by ComodityId) as data where retur > 0   " +
						...
						" select * into #factfinishingsubkon from ( select ComodityId,0 as SaldoQtyFin, 0 as QtyFin ,0 as FinishingTransfer,0 as AdjFin, 0 as Retur ,0 as QtyExport, 0 as QtySample, 0 as QtyOther ,0 as sewingretur,0 as QtyFinSub ,0 as cuttingSubkon, (case when ProcessDate >= @DateFrom then Qty else 0 end) as finoutSubkon from FactFinishingOutSubkon where   ProcessDate <= @DateTo  ) as data where finoutSubkon>0   " +
					...
					" group by comodityname,comodityCode order by comodityCode drop table #fin drop table #adjust drop table #returexpend drop table #factexpend drop table #finishingbarangjadi  drop table #factsewing     " +
