		...
		public IQueryable<FinishedGoodViewModel> GetFinishedGoodReport(DateTime? dateFrom, DateTime? dateTo, int offset)
		{
			...
			try
			{
					...
					new SqlConnection(connectionString))
				{
					...
					" union all  " +
					" select * from (select  ComodityId, sum (case when ProcessDate < @DateFrom then Qty else 0 end) as SaldoQtyFin,0 as QtyFin,0 as FinishingTransfer,0 as AdjFin,sum(case when ProcessDate >= @DateFrom then Qty else 0 end) as Retur,0 as QtyExport, 0 as QtySample, 0 as QtyOther ,0 as sewingretur ,0 as QtyFinSub,0 as cuttingSubkon,0 as finoutSubkon from  factreturExpend where   ProcessDate <= @DateTo group by ComodityId) as data where retur >0  " +
					" union all  " +
					" select * from (	select ComodityId,sum(case when ProcessDate < @DateFrom  then (-1*Qty) else 0 end) as SaldoQtyFin,0 as QtyFin,0 as FinishingTransfer,0 as AdjFin,0 as Retur,sum(case when ProcessDate >= @DateFrom  and ExpenditureType = 'E001' then Qty else 0 end) as QtyExport, sum(case when ProcessDate >= @DateFrom  and ExpenditureType = 'E003' then Qty else 0 end) as QtySample,sum (case when ProcessDate >= @DateFrom  and ExpenditureType = 'E002' then Qty else 0 end) as QtyOther  ,0 as sewingretur ,0 as QtyFinSub,0 as cuttingSubkon,0 as finoutSubkon   from (select ComodityId,Qty,ProcessDate,ExpenditureType from FactExpenditureGoods where   ProcessDate <= @DateTo   )data  group by ComodityId ) as data where saldoqtyfin <>0 " +

					" union all  " +
					" select * from (	 select  ComodityId,sum(case when ProcessDate < @DateFrom then Qty else 0 end) as SaldoQtyFin, sum(case when ProcessDate >= @DateFrom then Qty else 0 end)   as QtyFin,0 as FinishingTransfer,0 as AdjFin, 0 as Retur ,0 as QtyExport, 0 as QtySample, 0 as QtyOther ,0 as sewingretur, 0 as  QtyFinSub,	0 as cuttingSubkon ,0 as finoutSubkon 	from (select ProcessDate,ComodityId,UnitCode,UnitCodeTo,Qty from FactFinishingDetailBarangJadi where   ProcessDate <= @DateTo     and FinishingOutTo = 'GUDANG JADI'  ) data   group by comodityid ) as data where  saldoqtyfin >0  " +
				    " union all  " +
					 
					" select * from (select ComodityId, 0 as SaldoQtyFin, 0 as QtyFin, 0 as FinishingTransfer, 0 as AdjFin, 0 as Retur, 0 as QtyExport, 0 as QtySample, 0 as QtyOther, (case when ProcessDate >= @DateFrom and SewingOutTo = 'CUTTING' and UnitCode = UnitCodeTo then Qty else 0 end) as sewingretur,0 as QtyFinSub ,0 as cuttingSubkon, 0 as finoutSubkon from factsewing where ProcessDate <= @DateTo ) as data where sewingretur >0" +
				
					" union all  " +
					" select * from (select ComodityId,0 as SaldoQtyFin, 0 as QtyFin ,0 as FinishingTransfer,0 as AdjFin, 0 as Retur ,0 as QtyExport, 0 as QtySample, 0 as QtyOther ,0 as sewingretur,0 as QtyFinSub ,0 as cuttingSubkon, (case when ProcessDate >= @DateFrom then Qty else 0 end) as finoutSubkon from FactFinishingOutSubkon where   ProcessDate <= @DateTo  ) as data where finoutSubkon>0  " +
					" union all " +
					...
					" union all  " +
					" select * from (select   ComodityId,  0 as SaldoQtyFin,0 as QtyFin, isnull((select case when ProcessDate >=@DateFrom and UnitCode <> UnitCodeTo  and SewingOutTo='FINISHING' then Qty else 0 end    ),0) as FinishingTransfer,0 as AdjFin, 0 as Retur ,0 as QtyExport, 0 as QtySample, 0 as QtyOther ,0 as sewingretur,0 as QtyFinSub,0 as cuttingSubkon,0 as finoutSubkon from [FactSewingNotInSewingIn] where   ProcessDate <= @DateTo ) as data2 where  FinishingTransfer >0 ) as data join(select comodityId, comodityName, comodityCode from comodity ) comodity on data.comodityid = comodity.comodityId   " +
