...
{
    public class BC262Service : IBC262
    {
        ...
        public BC262Service(SupportDbContext context, IServiceProvider serviceProvider)
        {
        ...
        {
            IQueryable<TPBViewModelList> Query = dbSet.Where(s => s.kodeDokumen == "262" && s._IsDeleted == false).Select(m => new TPBViewModelList
            {
                Id = m.Id,
                nomorAju = m.nomorAju,
        ...
        public string Urut()
        {
            var Query = dbSet.Where(s => s.kodeDokumen == "262" && s._IsDeleted == false).OrderByDescending(x => x.nomorAju).Select(x => new { x.nomorAju }).FirstOrDefault();
            var NoUrut = "";
            if (Query != null)
            {
        ...
        public async Task<TPBHeader> ReadById(long id)
        {
            var model = await dbSet.Where(m => m.Id == id)
                ...
                .Include(m => m.bahanBaku)
                .Include(m => m.bahanBakuTarif)
                .Include(m => m.jaminan)
                .FirstOrDefaultAsync();
