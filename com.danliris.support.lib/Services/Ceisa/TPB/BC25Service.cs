...
{
    public class BC25Service : IBC25
    {
        ...
        public BC25Service(SupportDbContext context, IServiceProvider serviceProvider)
        {
        ...
        {
            IQueryable<TPBViewModelList> Query = dbSet.Where(s => s.kodeDokumen == "25" && s._IsDeleted == false).Select(m => new TPBViewModelList
            {
                Id = m.Id,
                nomorAju = m.nomorAju,
        ...
        public string Urut()
        {
            var Query = dbSet.Where(s => s.kodeDokumen == "25" && s._IsDeleted == false).OrderByDescending(x => x.nomorAju).Select(x => new { x.nomorAju }).FirstOrDefault();
            var NoUrut = "";
            if (Query != null)
            {
