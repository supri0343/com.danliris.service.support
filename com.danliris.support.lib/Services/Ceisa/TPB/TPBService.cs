        ...
        public async Task<MemoryStream> GetExcel(string noAju)
        {
            try
            {
                var data = await dbSet.Where(x => x.nomorAju == noAju)
                    .Include(x => x.barang)
                        .ThenInclude(b => b.barangTarif)
                    .Include(x => x.barang)
                        .ThenInclude(b => b.barangDokumen)
                    .Include(x => x.barang)
                        .ThenInclude(b => b.bahanBaku)
                            .ThenInclude(bb => bb.bahanBakuTarif)
                    .Include(x => x.entitas)
                    .Include(x => x.dokumen)
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    {
                        var tgl = a.tanggalIjinEntitas != null ? a.tanggalIjinEntitas.Value.ToString("yyyy-MM-dd") : "";
                        resultEntitas.Rows.Add(data.nomorAju, a.seriEntitas, a.kodeEntitas, a.kodeJenisIdentitas, a.nomorIdentitas,
                            a.namaEntitas,a.alamatEntitas, a.nibEntitas, a.kodeJenisApi , a.kodeStatus,
                            a.nomorIjinEntitas, tgl, a.kodeNegara, a.niperEntitas);
                    }
                    #endregion  
